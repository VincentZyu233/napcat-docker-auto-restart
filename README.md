...
- **防重启风暴**：
  ...
  - **假在线深度探测（仅诊断告警）**：调用需要服务端真实往返的只读接口（`get_cookies`）观察「连接还在但服务端是否仍响应」；该接口存在冷缓存/偶发长时间无响应（实测首次 12s+、偶发两次尝试都超时），因此同一轮内会重试且**任一次成功即视为健康**，并且**只告警、绝不触发重启**（避免误杀健康容器）；连续 3 轮失败才提示一次。
